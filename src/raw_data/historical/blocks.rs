...
#[derive(Debug)]
pub(crate) struct FullBlockRecord {
    pub number: u64,
    pub hash: [u8; 32],
...
#[derive(Debug)]
pub(crate) struct MinimalBlockRecord {
    pub number: u64,
    pub timestamp: u64,
    pub transaction_count: u32,
    pub transaction_hashes: Vec<String>,
...
pub(crate) fn process_single_block_full(
    block: &Block,
    block_number: u64,
...
pub(crate) fn build_block_schema(fields: &Option<Vec<BlockField>>) -> Arc<Schema> {
    match fields {
        Some(block_fields) => {
...
pub(crate) fn write_minimal_blocks_to_parquet(
...
pub(crate) fn write_full_blocks_to_parquet(
