...
// --- DhookTvlMetricsHandler ---

pub struct DhookTvlMetricsHandler {
    metadata_cache: Arc<PoolMetadataCache>,
    oracle_cache: Arc<OraclePriceCache>,
    chain_id: u64,
    db_pool: OnceLock<Pool>,
...
#[async_trait]
impl TransformationHandler for DhookTvlMetricsHandler {
    fn name(&self) -> &'static str {
        "DhookTvlMetricsHandler"
    }
...
impl EventHandler for DhookTvlMetricsHandler {
    fn triggers(&self) -> Vec<EventTrigger> {
        vec![EventTrigger::new(
            SOURCE,
            "Swap(address,(address,address,uint24,int24,address),bytes32,(bool,int256,uint160),int128,int128,bytes)",
    ...
    fn contiguous_handler_dependencies(&self) -> Vec<&'static str> {
        vec!["DopplerHookCreateHandler", "DhookLiquidityMetricsHandler"]
    }
}
...
pub fn register_handlers(
    ...
    registry.register_event_handler(DhookTvlMetricsHandler {
        metadata_cache: cache,
        oracle_cache,
        chain_id,
        db_pool: OnceLock::new(),
        tvl_config: TvlHandlerConfig {
            liquidity_source: "DhookLiquidityMetricsHandler",
            liquidity_source_version: 1,
            swap_source: "DhookSwapMetricsHandler",
