...
impl TransformationHandler for DopplerHookCreateHandler {
    ...
    async fn handle(
        ...
        for event in ctx.events_of_type("DopplerHookInitializer", "Create") {
            ...
            let pool_key = {
                let field_err = |field: &str, expected: &str| {
                    TransformationError::TypeConversion(format!(
                        "DopplerHookInitializer getState for {} field '{}': expected {} but got {:?} at block {} tx {}",
                ...
                let missing_err = |field: &str| {
                    TransformationError::MissingData(format!(
                        "DopplerHookInitializer getState for {} missing field '{}' at block {} tx {}. Available fields: {:?}",
            ...
            let get_positions_call = ctx.calls_of_type("DopplerHookInitializer", "getPositions")
                .filter(|call| call.trigger_log_index.unwrap() == event.log_index)
                .next()
                .ok_or_else(|| TransformationError::MissingData(format!(
...
impl EventHandler for DopplerHookCreateHandler {
    ...
    fn call_dependencies(&self) -> Vec<(String, String)> {
        vec![
            ...
            ("DopplerHookInitializer".to_string(), "getPositions".to_string()),
        ]
    }
}
