...
// --- DecayMulticurveTvlMetricsHandler ---

pub struct DecayMulticurveTvlMetricsHandler {
    metadata_cache: Arc<PoolMetadataCache>,
    oracle_cache: Arc<OraclePriceCache>,
    chain_id: u64,
    db_pool: OnceLock<Pool>,
...
#[async_trait]
impl TransformationHandler for DecayMulticurveTvlMetricsHandler {
    fn name(&self) -> &'static str {
        "DecayMulticurveTvlMetricsHandler"
    }
    ...
    async fn initialize(&self, db_pool: &DbPool) -> Result<(), TransformationError> {
        ...
        self.metadata_cache
            .load_into(db_pool, self.chain_id)
            .await?;
        tracing::info!("DecayMulticurveTvlMetricsHandler initialized");
        Ok(())
    }
}

impl EventHandler for DecayMulticurveTvlMetricsHandler {
    fn triggers(&self) -> Vec<EventTrigger> {
        vec![EventTrigger::new(
            SOURCE,
            "Swap(address,(address,address,uint24,int24,address),bytes32,(bool,int256,uint160),int128,int128,bytes)",
...
pub fn register_handlers(
    ...
    registry.register_event_handler(DecayMulticurveTvlMetricsHandler {
        metadata_cache: cache,
        oracle_cache,
        chain_id,
        db_pool: OnceLock::new(),
        tvl_config: TvlHandlerConfig {
            liquidity_source: "DecayMulticurveLiquidityMetricsHandler",
            liquidity_source_version: 1,
            swap_source: "DecayMulticurveSwapMetricsHandler",
